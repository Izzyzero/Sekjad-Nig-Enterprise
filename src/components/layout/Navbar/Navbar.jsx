...
export function Navbar({ open, setOpen, isAuthenticated = true, cartCount = 0, user = null, onLogout }) {
  ...
  const navBackground = scrolled ? 'bg-white shadow-sm' : 'bg-transparent'
  ...
  return (
    <>
      {/* ── Navbar ── */}
      <nav className={`fixed inset-x-0 top-9 z-50 transition-all duration-300 sm:top-8 ${navBackground}`}>
        <div className="mx-auto max-w-7xl px-5 sm:px-8">
          <div className="flex h-16 items-center justify-between lg:h-20">

            {/* Logo */}
            <a className="shrink-0 leading-none" href="#top" onClick={closeMenu}>
              ...
              {isAuthenticated ? (
                <>
                  ...
                  <div className="relative hidden lg:block" ref={dropdownRef}>
                    ...
                    {/* Dropdown panel */}
                    <div
                      className={`absolute right-0 mt-2 w-48 origin-top-right rounded-2xl border border-gray-100 bg-white shadow-lg transition-all duration-200 ${
                        accountOpen
